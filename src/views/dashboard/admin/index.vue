<template>
  <div class="dashboard-editor-container">
    <el-button @click="sendNode">发布</el-button>
...
export default {
  ...
  methods: {
    ...
    getNode() {
      return this.addRouters.map((item, index) => {
        const newVar = {
          'path': item.path,
        ...
        if (item.children) {
          newVar.children = this.getChildren(item.children)
        }
        return newVar
      }).filter(item => {
        return item.path !== '*'
